...
func RunHarvest(ctx context.Context, cfg HarvestConfig) (*HarvestResult, error) {
	...
	cmd := exec.CommandContext(harvestCtx, binPath, "--config", configPath)
	cmd.Dir = workDir
	...
	if err != nil {
		// On timeout, the tool may have produced partial output — continue parsing
		if harvestCtx.Err() == context.DeadlineExceeded {
			logger.Info("Proxy scraping reached %d-minute timeout — using partial results", timeoutMin)
		} else if ctx.Err() != nil {
			// Parent context cancelled (Ctrl+C)
			return nil, fmt.Errorf("cancelled: %w", ctx.Err())
		} else {
			logger.Warning("proxy-scraper-checker exited with error: %v", err)
			logger.FileDebug("proxy-scraper-checker output: %s", string(output))
		}
	}
